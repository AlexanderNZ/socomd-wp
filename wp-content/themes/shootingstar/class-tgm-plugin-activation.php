...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'shootingstar-install-plugins';
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'oops'                           => __( 'Something went wrong.', 'shootingstar' ),
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'shootingstar_dismissed_notice', true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            if ( isset( $_POST['action'] ) && 'shootingstar-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }
            ...
            <div class="shootingstar wrap">
                ...
                <form id="shootingstar-plugins" action="" method="post">
                    <input type="hidden" name="shootingstar-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['shootingstar-install'] ) && 'install-plugin' == $_GET['shootingstar-install'] ) ) {
                check_admin_referer( 'shootingstar-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'shootingstar-install' => 'install-plugin',
                        ),
                        network_admin_url( 'themes.php' )
                    ),
                    'shootingstar-install'
                );
                $method = ''; // Leave blank so WP_Filesystem can populate it as necessary.
                $fields = array( 'shootingstar-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'shootingstar' ) . '">' . __( 'Return to the Dashboard', 'shootingstar' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['shootingstar-activate'] ) && 'activate-plugin' == $_GET['shootingstar-activate'] ) ) {
                check_admin_referer( 'shootingstar-activate', 'shootingstar-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'shootingstar' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'shootingstar' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'shootingstar' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'shootingstar_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'shootingstar-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['shootingstar-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'shootingstar_dismissed_notice', 1 );
            }
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'shootingstar_dismissed_notice' );
    ...
    $shootingstar = TGM_Plugin_Activation::get_instance();
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'shootingstar' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'shootingstar' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'shootingstar' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'shootingstar' ) : __( 'Recommended', 'shootingstar' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'shootingstar' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'shootingstar' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'shootingstar' ) . ' %2$s">' . __( 'Install', 'shootingstar' ) . '</a>',
                        esc_url(
						    wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'shootingstar' ) . ' %2$s">' . __( 'Activate', 'shootingstar' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
                                    ...
                                    'plugin_name'          => urlencode( $item['sanitized_plugin'] ),
                                    'plugin_source'        => urlencode( $item['url'] ),
                                    'shootingstar-activate'       => 'activate-plugin',
                                    'shootingstar-activate-nonce' => urlencode( wp_create_nonce( 'shootingstar-activate' ) ),
                                ),
                                network_admin_url( 'themes.php' )
                            )
                        ),
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'shootingstar' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'shootingstar-bulk-install'  => __( 'Install', 'shootingstar' ),
                'shootingstar-bulk-activate' => __( 'Activate', 'shootingstar' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'shootingstar-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => urlencode( TGM_Plugin_Activation::$instance->menu ),
                            'shootingstar-action'  => 'install-selected',
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'shootingstar-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'shootingstar-empty';
                    $i++;
                }
            ...
            if ( 'shootingstar-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'shootingstar' ), $imploded );
                }
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'shootingstar' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'shootingstar' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'shootingstar' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'shootingstar' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'shootingstar' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'shootingstar' );
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    ...
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'shootingstar' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'shootingstar' ) . '</span><span class="hidden">' . __( 'Hide Details', 'shootingstar' ) . '</span>.</a>';
                ...
                // Default installation strings.
                    ...
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'shootingstar' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'shootingstar' ) . '</span><span class="hidden">' . __( 'Hide Details', 'shootingstar' ) . '</span>.</a>';
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'shootingstar' ) . '">' . __( 'Return to the Dashboard', 'shootingstar' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
